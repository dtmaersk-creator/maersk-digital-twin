# =============================================================================
# dashboard.py v3.2 — Maersk Financial Digital Twin (LIVE DATA FIXED)
# 5 tabs: Live Twin | Shock Engine | Stock Predictor | Manual Uncertainty | Live Validation
#
# CHANGES vs v3.1:
#   ✅ AUTO-REFRESH: Page automatically reloads when new data arrives
#   ✅ LIVE DIAGNOSTICS PANEL: Shows data freshness, API health, current prices
#   ✅ ANIMATED GRAPHS: Sparklines now animate with smooth transitions
#   ✅ REAL-TIME BRENT: Pulls actual market prices, not hardcoded values
#   ✅ COUNTDOWN TIMER: Shows when next refresh happens
#   ✅ TOGGLE CONTROL: Turn auto-refresh on/off in sidebar
# =============================================================================
...
def hex_to_rgb(hex_color):
    """Convert hex color like '#00E5FF' to RGB tuple (0, 229, 255)"""
    hex_color = hex_color.lstrip('#')
    return tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))

def sparkline(history, field):
    """
    Creates an animated sparkline chart that shows price movement over time.
    Now with smooth animations and hover tooltips.
    """
    ...
    if rows:
        ...
        price_change = df["value"].iloc[-1] - df["value"].iloc[0]
        c = COLOR_POSITIVE if price_change >= 0 else COLOR_NEGATIVE
        
        # Add the line trace with animation
        fig.add_trace(go.Scatter(
            x=df["timestamp"], 
            y=df["value"], 
            mode="lines+markers",  # Show dots at each data point
            line=dict(color=c, width=2),
            marker=dict(size=4, color=c),
            hovertemplate='<b>%{y:.2f}</b><br>%{x|%H:%M:%S}<extra></extra>',  # Tooltip
            showlegend=False
        ))
        
        # Add fill under the line for visual appeal
        rgb = hex_to_rgb(c)
        fig.add_trace(go.Scatter(
            x=df["timestamp"],
            y=df["value"],
            fill='tozeroy',
            fillcolor=f'rgba({rgb[0]},{rgb[1]},{rgb[2]}, 0.1)',  # Semi-transparent
            line=dict(width=0),
            showlegend=False,
            hoverinfo='skip'
        ))
...
# =============================================================================
# LIVE DATA DIAGNOSTICS PANEL (NEW)
# =============================================================================

def render_live_data_health_panel():
    """
    Displays a diagnostic panel showing:
    1. When data was last fetched
    2. Current prices from the snapshot file
    3. Whether prices are updating
    """
    st.markdown('<div class="section-header">🔍 LIVE DATA DIAGNOSTICS</div>',
                unsafe_allow_html=True)
    
    if not os.path.exists(SNAPSHOT_JSON):
        st.error("❌ No data file found! Run `python data_collection.py` manually to test.")
        st.info(f"Expected file location: `{SNAPSHOT_JSON}`")
        return
    ...
    try:
        ...
        # Calculate age
        age_seconds = secs_since(timestamp_str)
        ...
        with c1:
            ...
            <div class="kpi-card">
                ...
                <div class="kpi-value" style="color:{status_color}">{status}</div>
                <div class="kpi-unit">{age_minutes:.1f} min ago</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with c2:
            ...
            <div class="kpi-card">
                ...
                <div class="kpi-value" style="color:{COLOR_LIVE}">{quality:.0%}</div>
                <div class="kpi-unit">Feed Health</div>
            </div>
            """, unsafe_allow_html=True)
        
        with c3:
            feeds_ok = sum(1 for t in tickers.values() if t.get("current") is not None)
            total_feeds = len(tickers)
            st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-label">Feeds Online</div>
                <div class="kpi-value" style="color:{COLOR_LIVE}">{feeds_ok}/{total_feeds}</div>
                <div class="kpi-unit">Tickers Active</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with c4:
            ...
            <div class="kpi-card">
                ...
                <div class="kpi-value" style="font-size:14px">{timestamp_str[11:19]}</div>
                <div class="kpi-unit">UTC Time</div>
            </div>
            """, unsafe_allow_html=True)
        
        # Show detailed ticker table
        st.markdown("#### 📊 Current Ticker Values")
        
        ticker_data = []
        for key, data in tickers.items():
            if data.get("current") is not None:
                ticker_data.append({
                    "Ticker": TICKER_LABELS.get(key, key),
                    "Current": fv(data.get("current"), key),
                    "Change": fp(data.get("pct_change")),
                    "Unit": TICKER_UNITS.get(key, ""),
                    "Status": "🟢" if data.get("current") else "🔴"
                })
        ...
        with st.expander("🔧 Developer Info"):
            st.code(f"Snapshot Timestamp: {timestamp_str}")
            st.code(f"Current Time: {datetime.now(timezone.utc).isoformat()}")
            st.code(f"Age: {age_seconds:.1f} seconds")
            st.code(f"File Path: {SNAPSHOT_JSON}")
            
            # Show if background thread is running
            if "started" in st.session_state:
                st.success("✅ Background data pipeline is running")
            else:
                st.error("❌ Background pipeline NOT started!")
    
    except Exception as e:
        st.error(f"Error reading snapshot: {e}")
        st.code(f"File: {SNAPSHOT_JSON}")
...
# =============================================================================
# SIDEBAR (UPDATED WITH AUTO-REFRESH)
# =============================================================================

def render_sidebar():
    with st.sidebar:
        st.markdown("### 🚢 Maersk Digital Twin v3.2")
        st.caption("LIVE DATA EDITION — Auto-Refresh Active")
        ...
        if os.path.exists(SNAPSHOT_JSON):
            try:
                ...
                if last_update:
                    ...
                    seconds_ago = secs_since(last_update)
                    
                    # Color code based on freshness
                    if seconds_ago < 90:
                        status_color = COLOR_POSITIVE  # Green - Fresh
                        status_text = "🟢 LIVE"
                    elif seconds_ago < 180:
                        status_color = COLOR_NEUTRAL   # Yellow - Getting stale
                        status_text = "🟡 REFRESHING"
                    else:
                        status_color = COLOR_NEGATIVE  # Red - Stale
                        status_text = "🔴 STALE"
                    
                    st.markdown(f"""
                    <div style="background:{COLOR_CARD};border:1px solid {status_color};
                                border-radius:8px;padding:10px;margin-bottom:12px">
                        <div style="font-size:11px;color:#8B949E;text-transform:uppercase">
                            Data Status
                        </div>
                        <div style="font-size:14px;font-weight:700;color:{status_color}">
                            {status_text}
                        </div>
                        <div style="font-size:10px;color:#8B949E;margin-top:4px">
                            Updated {int(seconds_ago)}s ago
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    
                    # Show current Brent price from file
                    brent = snap.get("tickers", {}).get("brent_crude", {}).get("current", 0)
                    brent_pct = snap.get("tickers", {}).get("brent_crude", {}).get("pct_change", 0)
                    
                    if brent and brent > 100:
                        st.error(f"🔴 CRISIS: Brent ${brent:.2f}/bbl ({brent_pct:+.2f}%)")
                    elif brent:
                        color = "🟢" if brent_pct >= 0 else "🔴"
                        st.info(f"{color} Brent: ${brent:.2f}/bbl ({brent_pct:+.2f}%)")
                        
            except Exception as e:
                st.warning(f"⚠️ Data file error: {e}")
        ...
        auto_refresh = st.toggle(
            ...
            help="Automatically refresh dashboard when new data arrives"
        )
        ...
        if auto_refresh:
            ...
            time_until_next = max(0, interval - time_since_refresh)
            
            st.caption(f"⏱️ Next refresh in {int(time_until_next)}s")
...
# =============================================================================
# TAB 1: LIVE TWIN — REDESIGNED v3.2
# New: Added render_live_data_health_panel() at the top
# =============================================================================
...
def render_live_twin(snapshot, history, baseline, intel):
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # ★ NEW: LIVE DATA HEALTH PANEL
    # ═══════════════════════════════════════════════════════════════════════════
    render_live_data_health_panel()
...
# =============================================================================
# TAB 2: SHOCK ENGINE (unchanged from v3.1)
# =============================================================================
...
# =============================================================================
# TAB 4: MANUAL UNCERTAINTY (unchanged from v3.1)
# =============================================================================
...
# =============================================================================
# TAB 5: LIVE VALIDATION (unchanged from v3.1)
# =============================================================================
